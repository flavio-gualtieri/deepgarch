...
class GARCHFamily(VolatilityModel):
    ...
    def loglikelihood(self, returns: Tensor) -> Tensor:
        ...
        log_2pi = torch.log(torch.tensor(2 * torch.pi, dtype=returns.dtype, device=returns.device))
    ...
    def forecast(self, returns: Tensor, h: int) -> Tensor:
        ...
        for i in range(h):
            # Beyond the sample, E[ε²_{t}] = σ²_{t} under Gaussian innovations
            sigma2 = self.variance_equation(len(returns) + i, last_return, sigma2)
            forecasts[i] = sigma2
            last_return = torch.sqrt(sigma2)  # stand-in: E[|ε|] under Gaussian
