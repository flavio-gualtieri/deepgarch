...
class MarketData:
    ...
    def __init__(
        ...
        self.train_return_mean: float | None = None
    ...
    def load(self) -> "MarketData":
        ...
        # Demean returns using only the train-period mean, applied as a fixed
        # constant to the whole frame — demeaning on the full sample (as the
        # previous version did) leaks val/test information into every training
        # observation via the mean subtracted from day one.
        train_mean = frame.loc[frame.index < self.val_start, "returns"].mean()
        frame["returns"] = frame["returns"] - train_mean
        self.train_return_mean = float(train_mean)
